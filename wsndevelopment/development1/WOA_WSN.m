...
for Exper_index = 1:Exper_num
    ...
    %计算每个网格的表面积
    sur_area_grid = zeros(L/0.1,W/0.1);%用于存每个小曲面的表面积 最底下的为第一行
    for i=1:L/0.1
        for j=1:W/0.1
    ...
    Grid_cen_x = zeros(1,L/0.1);%网格中心点x坐标
    Grid_cen_y = zeros(1,W/0.1);%网格中心点y坐标
    %前后两者相加之和除以2
    for i=1:L/0.1
    ...
    while iter <= ger
        ...
        a=2-iter*((2)/ger);    % 式（2.3）中，a从2线性减小到0
       %a2=-1+iter*((-1)/ger);  % a2从-1到-2线性递减，用以计算公式（3.12）中的t 
       % 更新种群位置
       for i = 1:sizepop
           % Y = wolf_pop(:,i);
          ...
          for j = 1:dimension
            if p < 0.5
                if abs(A) >= 1
                    ...
                    X_rand = wolf_pop(:,rand_leader_index);
                    D_X_rand = abs(C*X_rand(j,:)-wolf_pop);          % 公式(2.7)
                    pop_new = X_rand - A*D_X_rand;                % 公式(2.8)
                elseif abs(A) < 1
                    D_Leader = abs(C*wolf_leader_pops-wolf_pop);     % 公式(2.1)
                    pop_new = wolf_leader_pops-A*D_Leader;               % 公式(2.2)
                end
            else
                distance2Leader = abs(wolf_leader_pops-wolf_pop);
                % 公式(2.5)
                pop_new = distance2Leader*exp(b.*l).*cos(l.*2*pi)+wolf_leader_pops;
            end
          end
       end
...
%调用最后一个种群连通的判断函数
[liantonglv,fugailv]=last_pop_connec(wolf_pop,sizepop,all_sur_area);

data(mm,:) = [liantonglv fugailv];
end
% save data.mat data