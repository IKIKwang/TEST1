    ...
    while iter <= ger
        ...
        record_pop_ave(1,iter) = sum_fitness/sizepop;%算出该种群的平均适应度
         ...
         for i = 1:sizepop
           ...
           r1=rand(1);
           r2=rand(1);
           %V(:,i)=vmax*rand(dimension,N,sizepop);
           V =   c1*r1*(wolf_leader_pops-wolf_pop)+c2*r2*(wolf_leader_pops-wolf_pop);
           V_UB=V(:,i)>vmax;
           V_LB=V(:,i)<vmin;
           V = (V(:,i).*(~(V_UB+V_LB)))+vmax.*V_UB+vmin.*V_LB;  

           Xnew = wolf_pop + V;
           Flag_UB=Xnew(:,i)>ub;           
           Flag_LB=Xnew(:,i)<lb;           
           Xnew = (Xnew.*(~(Flag_UB+Flag_LB)))+ub.*Flag_UB+lb.*Flag_LB;  
        ...
        for i=1:sizepop
            for j=1:dimension-1%第三维得计算
                for k=1:N
                    if Xnew(j,k,i) > limit(1,2) || Xnew(j,k,i) < limit(1,1)
                        Xnew(j,k,i) = wolf_leader_pops(j,k,i);
    %                     pop_new(3,k,i) = ma_an([pop_new(1,k,i),pop_new(2,k,i)]);%暂时先不更新第三维位置
        ...
        for i=1:sizepop
            ...
            for k=1:swap_node_num
                wolf_pop(:,node_index(1,k),i) = Xnew(:,node_index(1,k),i);
