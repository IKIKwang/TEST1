...
%               newPopPos(1,:)=BestX+rand(1,Dim).*(BestX-PopPos(1,:))+Beta*(BestX-PopPos(1,:)); %Equation (4)
              ...
              IndivRand=rand(dimension,N,sizepop).*(Up-Low)+Low;     
...
%     for i=1:sizepop
%         wolf_fitness(1,i) = get_Grid_cover_unit_and_rate(pop_new(:,:,i),all_sur_area);
%     end
%    
%     [fitness_max,max_index] = max(wolf_fitness);%对适应值进行排序
%     wolf_leader = wolf_pop(:,:,max_index);%领头狼
...
save mrfo_best_indivi.mat best_indivi;
%调用最后一个种群连通的判断函数
